...
function problemA () {
  ...
  // ???
  //
  var  stansas=[promisifiedReadFile('poem-two/stanza-01.txt'), promisifiedReadFile('poem-two/stanza-02.txt')]
  Promise.all(stansas).then(stanza => {blue(stanza[0]),blue(stanza[1]),console.log('done')})
...
function problemB () {
  ...
  // promise version
  // ???
 var pro=filenames.map(res =>promisifiedReadFile(res)) ;
  Promise.all(pro).then(stanza =>{stanza.forEach(e => blue(e)),console.log('done')})
}
...
function problemC () {
  ...
  let pro=filenames.map((res) => promisifiedReadFile(res)) ;
  Promise.each(pro, (dato)=> blue(dato))
...
function problemD () {
  ...
  let pro=filenames.map((res) => promisifiedReadFile(res)) ;
  Promise.each(pro, (dato)=> blue(dato))
  .then((value)=> console.log('done')).catch(err=>{(magenta(new Error(err)),console.log('done'))})   
}
